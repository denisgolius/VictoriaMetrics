...
// getAdjustedTimestamp returns the current timestamp adjusted by a global offset.
//
...
func (q *Query) AddTimeFilter(start, end int64) {
	...
	minTimestamp := start - offset
	maxTimestamp := getMatchingEndTime(end, string(endStr)) - offset
	ft := &filterTime{
